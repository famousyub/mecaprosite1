...
<!-- Inventory -->
   <?php
$iQuery = $db->query("SELECT * FROM products WHERE deleted = 0 AND myquant<=threshold");
$lowItems = array();
while ($product = mysqli_fetch_assoc($iQuery)) {
    ...
    // }
		...
		//$myproducts = $product['myquant'];
		//foreach ($myproducts as $myproduct) {
		    if ($product['myquant'] <= $product['threshold']) {
		    $cat = get_category($product['categories']);
		    $item = array(
		        'title' => $product['title'],
		        //'size' => $myproduct['size'],
		        'quantity' => $product['myquant'],
